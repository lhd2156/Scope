...
export const useUserStore = defineStore('user', () => {
  ...
  async function fetchCurrentProfile() {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not load your profile right now.');
      throw nextError;
    } finally {
      loading.value = false;
    }
  ...
  async function fetchProfile(userId: string) {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not load that explorer profile right now.');
      throw nextError;
    } finally {
      loading.value = false;
    }
  ...
  async function fetchStats(userId = profile.value?.id ?? authStore.currentUser?.id ?? '') {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not load user stats right now.');
      throw nextError;
    } finally {
      loading.value = false;
    }
  ...
  async function saveProfile(updates: UpdateUserProfileInput, userId = profile.value?.id ?? authStore.currentUser?.id ?? '') {
    if (!userId) {
      throw new Error('No Atlas user is selected for this profile update.');
    }
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not update that profile right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
  ...
  async function searchProfiles(query: string, page = 1, pageSize = 10) {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not search explorers right now.');
      throw nextError;
    } finally {
      loading.value = false;
    }
  ...
  async function deactivateProfile(userId = profile.value?.id ?? authStore.currentUser?.id ?? '') {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not deactivate that profile right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
