package com.scope.mobile.ui.explore
...
import com.scope.mobile.ui.home.SpotCard
...
fun ExploreScreen(
    ...
    Column(
        ...
        TextField(
            ...
            placeholder = { Text("Search spots, cities, vibes", color = ScopeTokens.TextMuted) },
            leadingIcon = { Icon(Icons.Filled.Search, contentDescription = null, tint = ScopeTokens.TextMuted) },
            singleLine = true,
            shape = RoundedCornerShape(12.dp),
            colors = TextFieldDefaults.colors(
                focusedContainerColor = ScopeTokens.BgSecondary,
                unfocusedContainerColor = ScopeTokens.BgSecondary,
                focusedIndicatorColor = Color.Transparent,
                unfocusedIndicatorColor = Color.Transparent,
                focusedTextColor = ScopeTokens.TextPrimary,
                unfocusedTextColor = ScopeTokens.TextPrimary
            ),
        ...
        when {
            state.isLoading && state.spots.isEmpty() ->
                EmptyHint("Searching the Scope…")
            state.error != null -> Text(state.error!!, color = ScopeTokens.Danger)
            state.spots.isEmpty() ->
                EmptyHint("No spots match your filters. Try another category.")
            else -> LazyColumn(
                verticalArrangement = Arrangement.spacedBy(ScopeSpacing.Base),
                modifier = Modifier.fillMaxWidth()
            ) {
...
private fun CategoryPill(label: String, active: Boolean, onClick: () -> Unit) {
    val bg = if (active) ScopeTokens.AccentTeal else ScopeTokens.BgSecondary
    val fg = if (active) Color(0xFF0B1020) else ScopeTokens.TextPrimary

    Row(
        Modifier
            .clickable(onClick = onClick)
            .background(bg, RoundedCornerShape(999.dp))
            .border(1.dp, if (active) Color.Transparent else ScopeTokens.Border, RoundedCornerShape(999.dp))
            .padding(horizontal = ScopeSpacing.Base, vertical = ScopeSpacing.Sm)
    ) {
