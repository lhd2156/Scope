...
const APP_SHELL_CACHE = 'scope-shell-v2';
const STATIC_ASSET_CACHE = 'scope-static-v2';
const NAVIGATION_CACHE = 'scope-navigation-v2';
...
describe('service worker offline caching', () => {
  ...
  it('cleans old cache versions during activation and claims open clients', async () => {
    const harness = createServiceWorkerHarness();
    harness.cacheStores.set('scope-shell-v1', new Map([['/', new Response('legacy')]]));
    ...
    expect(harness.caches.delete).toHaveBeenCalledWith('scope-shell-v1');
    expect(harness.cacheStores.has('scope-shell-v1')).toBe(false);
