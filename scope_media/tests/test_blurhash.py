...
def compiled_library() -> ctypes.CDLL:
    if shutil.which('cl') is None and shutil.which('gcc') is None:
        pytest.skip('No C compiler found in PATH, skipping native scope_media blurhash tests.')
...
def test_encode_blurhash_rejects_invalid_arguments(compiled_library: ctypes.CDLL) -> None:
    ...
    assert compiled_library.scope_media_encode_blurhash(None, 2, 2, output, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.scope_media_encode_blurhash(ctypes.byref(image), 0, 2, output, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.scope_media_encode_blurhash(ctypes.byref(image), 2, 0, output, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.scope_media_encode_blurhash(ctypes.byref(image), 10, 2, output, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.scope_media_encode_blurhash(ctypes.byref(image), 2, 10, output, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.scope_media_encode_blurhash(ctypes.byref(image), 2, 2, None, ctypes.sizeof(output)) == STATUS_INVALID_ARGUMENT

    too_short_output = ctypes.create_string_buffer(_expected_blurhash_length(2, 2))
    assert compiled_library.scope_media_encode_blurhash(
        ctypes.byref(image),
        2,
        2,
        too_short_output,
