...
INTERACTION_AFFINITY_WEIGHT = 0.18
FRIEND_AFFINITY_WEIGHT = 0.10
COLLABORATIVE_WEIGHT = 0.08
GEO_PROXIMITY_WEIGHT = 0.07
POPULARITY_WEIGHT = 0.07
QUALITY_WEIGHT = 0.05

# Cold-start profile: no history at all. Surface well-reviewed, well-liked
# spots first; skip personal-signal channels that would all evaluate to 0.
# These still sum to 1.0 so the score range stays consistent with warm scoring.
COLD_START_TEXT_WEIGHT = 0.25
COLD_START_POPULARITY_WEIGHT = 0.35
COLD_START_QUALITY_WEIGHT = 0.25
COLD_START_GEO_WEIGHT = 0.05
COLD_START_FRIEND_WEIGHT = 0.10

# Soft cap: 5 friend-likes on a spot maxes out the friend signal.
FRIEND_SIGNAL_CAP = 5
...
MMR_LAMBDA = 0.7
# Cold-start lists are served to users with no personal signal, so we lean
# much harder on diversity to avoid handing them six variations of the same
# "top global spot" category.
COLD_START_MMR_LAMBDA = 0.4
...
class RecommendationEngine:
    ...
    def recommend_spots(self, user_id: str, liked_spot_ids: list[str], interests: list[str], limit: int) -> list[dict]:
        source_spots = self.content_client.get_all_spots()
        if not source_spots:
            return []

        similarity_index = self._get_spot_similarity_index(source_spots)
        ...
        # the first-session experience surfaces the "must-see" spread rather
        ...
        if len(scored) >= MMR_MIN_CANDIDATES:
            mmr_lambda = COLD_START_MMR_LAMBDA if is_cold_start else MMR_LAMBDA
            ranked = self._apply_mmr(scored, limit, mmr_lambda=mmr_lambda)
        else:
            ranked = scored[:limit]
    ...
    def _build_reason(
        ...
        if is_cold_start:
            reason = f"Popular {spot.category} pick with a {spot.vibe} vibe"
            if friend_component > 0.0:
                reason = f"{reason}; your friends like it too"
            return reason
        reason = f"Matches interests in {spot.category} with {spot.vibe} vibe"
        if friend_component > 0.0:
            reason = f"{reason}; your friends are into this"
    ...
    def _get_spot_similarity_index(self, spots: list[Spot] | None = None) -> SpotSimilarityIndex:
        spots = self.content_client.get_all_spots() if spots is None else spots
