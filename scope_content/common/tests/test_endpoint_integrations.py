...
def test_trip_member_and_trip_mutation_endpoints_cover_happy_and_forbidden_paths(auth_header, trip):
    ...
    # A non-member attacking a private trip should not be able to distinguish
    # "trip does not exist" from "you are not allowed"; TripDetailView's
    # queryset filter hides the trip and we expect 404 NOT_FOUND.
    assert forbidden_delete_response.status_code == 404
    assert forbidden_delete_response.json()['error']['code'] == 'NOT_FOUND'
    assert delete_response.status_code == 204
