<template>
  ...
  >
    <p class="route-error-boundary__eyebrow">Atlas shell</p>
...
const fallbackMessage = computed(() => {
  ...
  if (errorMessage && CHUNK_ERROR_PATTERNS.some((pattern) => pattern.test(errorMessage))) {
    return 'Atlas could not finish loading that route bundle. Try this view again to request a clean reload.';
  }

  return 'A page-level error interrupted this workspace. Try this view again or head back home while Atlas recovers.';
});
