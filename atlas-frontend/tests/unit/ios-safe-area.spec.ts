...
describe('iOS safe-area and touch-action polish', () => {
  it('opts the app shell into viewport-fit=cover for standalone Safari PWAs', () => {
  ...
  it('keeps the fixed navbar and mobile map chrome clear of iOS cutouts', () => {
    ...
    expect(mapPageVue).toContain('--atlas-map-controls-right: max(var(--space-3), var(--safe-area-right));');
    expect(mapPageVue).toContain('--atlas-map-controls-panel-left: max(var(--space-3), var(--safe-area-left));');
