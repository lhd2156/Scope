import {
  ...
  getAtlasWasmModuleInfo,
  pingAtlasWasmRuntime,
  resetAtlasWasmRuntimeForTests,
...
describe('wasmService', () => {
  ...
  it('falls back cleanly when compiled wasm assets are unavailable', async () => {
    const moduleInfo = await getAtlasWasmModuleInfo();
    const pingValue = await pingAtlasWasmRuntime();
    ...
    expect(pingValue).toBe('atlas-wasm-js-fallback-ready');
  });
