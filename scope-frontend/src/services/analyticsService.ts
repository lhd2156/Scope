...
export type ScopeAiInteractionSource = 'typed' | 'suggestion' | 'route-action' | 'place-search';

export interface AnalyticsScopeAiInteractionPayload extends AnalyticsUserActionContext {
  interactionId: string;
  source: ScopeAiInteractionSource;
  prompt: string;
  assistantResponse?: string;
  responseKind: 'text' | 'places' | 'error';
...
export class AnalyticsPageEngagementTracker {
  ...
  beginPage(context: AnalyticsRouteContext): void {
    ...
    this.activeSession = {
      ...context,
      key: pageKey,
      isVisible: this.doc?.hidden !== true,
...
function compactTrainingText(value: string, maxLength = 600): string {
  return value.replace(/\s+/g, ' ').trim().slice(0, maxLength);
}

export function trackScopeAiInteraction(
  payload: AnalyticsScopeAiInteractionPayload,
  tracker?: AnalyticsEventTracker,
): void {
  const promptSample = compactTrainingText(payload.prompt);
  const assistantSample = payload.assistantResponse
    ? compactTrainingText(payload.assistantResponse)
    : undefined;
  ...
  resolveEventTracker(tracker).trackEvent({
    ...
    value: payload.conversationTurnCount,
    path: payload.path,
    title: payload.title,
    routeName: payload.routeName,
    metadata: {
      ...
      assistantLength: payload.assistantResponse?.trim().length,
      responseKind: payload.responseKind,
      responseModel: payload.responseModel,
      routeActionReason: payload.routeActionReason,
      placeResultCount: payload.placeResultCount,
      conversationTurnCount: payload.conversationTurnCount,
      hasStart: payload.hasStart,
      hasEnd: payload.hasEnd,
      stopCount: payload.stopCount,
      interestCount: payload.interestCount,
      groupSize: payload.groupSize,
      pace: payload.pace,
      budgetFloor: payload.budgetFloor,
      budget: payload.budget,
