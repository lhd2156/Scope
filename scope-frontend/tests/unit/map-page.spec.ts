...
const { mapInteractionTrackMock, mapStoreMock, onboardingStoreMock, resolveRoadRouteMock, spotsStoreMock, tripsStoreMock } = vi.hoisted(() => ({
    ...
    global: {
      stubs: {
        ...
        MapView: options?.mapViewStub ?? {
          props: ['spots', 'routePoints', 'routeGeometry', 'selectedSpotId'],
          template: '<div data-test="map-view-stub">{{ spots.length }} spots / {{ routePoints.length }} route points / {{ routeGeometry.length }} route coords / {{ selectedSpotId }}</div>',
        },
      },
    },
  });
...
describe('MapPage', () => {
  ...
  it('loads the premium sidebar workspace with category chips, route preview, and selected spot details', async () => {
    ...
    expect(wrapper.find('[data-test="map-view-stub"]').text()).toContain('2 spots / 2 route points / 2 route coords / spot-1');
  ...
  it('keeps an accurately measured empty viewport instead of falling back to every active pin', async () => {
    mapStoreMock.visibleSpotIds = [];
    mapStoreMock.visibleSpotIdsMeasured = true;
    mapStoreMock.selectedSpotId = '';
    ...
    expect(wrapper.findAll('button.visible-item')).toHaveLength(0);
    expect(wrapper.text()).toContain('No pins match this category mix');
  });
