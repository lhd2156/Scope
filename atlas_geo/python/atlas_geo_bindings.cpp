...
std::string coordinate_repr(const atlas::geo::Coordinate& coordinate) {
    std::ostringstream stream;
    stream << "Coordinate(latitude=" << coordinate.latitude << ", longitude=" << coordinate.longitude << ")";
    return stream.str();
}

std::string spatial_point_repr(const atlas::geo::SpatialPoint& point) {
    std::ostringstream stream;
    stream << "SpatialPoint(id='" << point.id << "', coordinate=" << coordinate_repr(point.coordinate) << ")";
    return stream.str();
}

std::string graph_node_repr(const atlas::geo::GraphNode& node) {
    std::ostringstream stream;
    stream << "GraphNode(id='" << node.id << "', coordinate=" << coordinate_repr(node.coordinate) << ")";
    return stream.str();
}

std::string graph_edge_repr(const atlas::geo::GraphEdge& edge) {
    std::ostringstream stream;
...
std::string viewport_repr(const atlas::geo::Viewport& viewport) {
    std::ostringstream stream;
...
PYBIND11_MODULE(_atlas_geo, module) {
    using atlas::geo::Coordinate;
    using atlas::geo::GraphEdge;
    using atlas::geo::GraphNode;
    using atlas::geo::NearestNeighbor;
    using atlas::geo::PathGraph;
    using atlas::geo::PathResult;
    using atlas::geo::RTreeIndex;
    using atlas::geo::SpatialPoint;
    using atlas::geo::Viewport;
    using atlas::geo::ViewportCluster;
    using atlas::geo::ViewportClusteringOptions;

    module.doc() = "pybind11 bindings for the atlas_geo native geospatial engine";
    module.attr("__version__") = py::str(atlas::geo::version());
    ...
    module.def("version", &atlas::geo::version);
    module.def("is_coordinate_valid", &atlas::geo::is_coordinate_valid, py::arg("coordinate"));
    module.def("haversine_distance_km", &atlas::geo::haversine_distance_km, py::arg("from_coordinate"), py::arg("to_coordinate"));
    module.def(
        "haversine_distance_meters",
        &atlas::geo::haversine_distance_meters,
        py::arg("from_coordinate"),
        py::arg("to_coordinate"));
    module.def("convex_hull", &atlas::geo::convex_hull, py::arg("points"));
    module.def("is_viewport_valid", &atlas::geo::is_viewport_valid, py::arg("viewport"));
    module.def("viewport_contains", &atlas::geo::viewport_contains, py::arg("viewport"), py::arg("coordinate"));
    module.def(
        "cluster_points_in_viewport",
        &atlas::geo::cluster_points_in_viewport,
        py::arg("points"),
        py::arg("viewport"),
