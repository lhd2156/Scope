...
def test_vibe_matcher_returns_empty_when_catalog_is_empty():
    from app.services.content_client import FixtureContentServiceClient
    from app.services.vibe_matcher import VibeMatcher

    matcher = VibeMatcher(FixtureContentServiceClient(spots=[]))

    assert matcher.match("quiet sunset overlook", 5) == []
