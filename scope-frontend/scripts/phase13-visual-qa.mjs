...
const BASE_URL = process.env.SCOPE_VISUAL_BASE_URL ?? 'http://127.0.0.1:4184';
const CUSTOM_EXECUTABLE = process.env.SCOPE_VISUAL_BROWSER_EXECUTABLE ?? '';
...
async function createContext(browser, theme, options = {}) {
  ...
  await context.addInitScript((themeMode) => {
    localStorage.setItem('scope-theme', themeMode);
    window.__SCOPE_VISUAL_QA__ = true;
  }, theme);
