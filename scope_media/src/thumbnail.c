...
static scope_media_status scope_media_allocate_output(
    uint32_t width,
    uint32_t height,
    uint8_t channels,
...
static double scope_media_clamp_double(double value, double minimum, double maximum) {
    if (value < minimum) {
        return minimum;
    }
...
static uint32_t scope_media_round_dimension(double value) {
    uint32_t rounded = (uint32_t)(value + 0.5);
    return rounded == 0 ? 1u : rounded;
}

static void scope_media_compute_thumbnail_size(
    ...
    *target_width = scope_media_round_dimension((double)input->width * scale);
    *target_height = scope_media_round_dimension((double)input->height * scale);
...
SCOPE_MEDIA_API scope_media_status scope_media_generate_thumbnail(
    ...
    uint32_t target_width;
    uint32_t target_height;
    scope_media_status allocate_status;
    uint32_t y;

    if (!scope_media_validate_image(input) || options == NULL || output == NULL
    ...
    if (output_channels != input->channels) {
        return SCOPE_MEDIA_STATUS_NOT_IMPLEMENTED;
    }

    scope_media_compute_thumbnail_size(input, options, &target_width, &target_height);
    allocate_status = scope_media_allocate_output(target_width, target_height, output_channels, output);
    if (allocate_status != SCOPE_MEDIA_STATUS_OK) {
    ...
    for (y = 0; y < target_height; ++y) {
        ...
        source_y = scope_media_clamp_double(source_y, 0.0, (double)input->height - 1.0);
        y0 = (size_t)source_y;
        ...
        for (x = 0; x < target_width; ++x) {
            ...
            source_x = scope_media_clamp_double(source_x, 0.0, (double)input->width - 1.0);
            x0 = (size_t)source_x;
