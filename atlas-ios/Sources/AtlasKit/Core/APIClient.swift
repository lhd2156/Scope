...
/// Thin wrapper around `URLSession` that understands the Atlas `ApiResponse<T>`
...
public actor APIClient {
    ...
    private let refresh: (@Sendable () async throws -> AtlasTokens)?
    private var refreshTask: Task<AtlasTokens, Error>?

    public init(
        baseURL: URL,
        session: URLSession = .shared,
        tokens: any TokenStoring,
        refresh: (@Sendable () async throws -> AtlasTokens)? = nil
    ) {
    ...
    private func buildRequest<B: Encodable & Sendable>(
        ...
        request.setValue("atlas-ios/1.0", forHTTPHeaderField: "User-Agent")
    ...
    private func refreshTokens() async throws -> AtlasTokens {
...
/// Response envelope shared across all Atlas backends.
public struct ApiEnvelope<T: Decodable>: Decodable {
    public let data: T?
    public let meta: AtlasMeta?
}

public struct AtlasMeta: Decodable, Sendable {
    public let page: Int?
