...
final class APIClientTests: XCTestCase {
    func testEnvelopeDecodingUnwrapsData() async throws {
        ...
        await tokens.save(ScopeTokens(accessToken: "access-1", refreshToken: "refresh-1"))
        let user: ScopeUser = try await client.get("/api/core/auth/me")
    ...
    func testRawJSONAlsoDecodes() async throws {
        ...
        let users: [ScopeUser] = try await client.get("/api/core/users/search", query: ["q": "u"])
    ...
    func testUnauthorizedMaps401ToAPIError() async throws {
        ...
        do {
            let _: ScopeUser = try await client.get("/api/core/auth/me")
    ...
    func testServerErrorSurfacesMessage() async throws {
        ...
        do {
            let _: ScopeUser = try await client.get("/api/core/health")
    ...
    private func makeClient() -> (APIClient, InMemoryTokenStore) {
        ...
        let client = APIClient(baseURL: URL(string: "https://scope.test")!, session: session, tokens: tokens)
