...
def _map_content_row(row: dict[str, Any]) -> Spot:
    ...
    likes_count = int(row.get("likesCount") or row.get("likes_count") or 0)
    photos_count = int(row.get("photosCount") or row.get("photos_count") or 0)
    ...
    return Spot(
        spot_id=str(row.get("id") or row.get("spotId") or row.get("spot_id") or ""),
...
class HttpContentServiceClient:
    ...
    def _fetch_spots(self, params: dict[str, Any]) -> list[Spot]:
        ...
        if isinstance(payload, dict):
            ...
            rows = []
        return [_map_content_row(row) for row in rows if isinstance(row, dict)]
...
_CONTENT_CLIENT_EXTENSION_KEY = "atlas_intel_content_client"
