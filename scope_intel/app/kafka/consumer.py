...
SPOT_FEATURE_TOPICS = ("spot.created", "spot.updated")
FRIEND_REMOVAL_TOPICS = ("friend.removed", "friend.rejected")
...
class KafkaSpotFeatureConsumer:
    ...
    def handle_message(self, topic: str, payload: dict, event_id: str | None = None) -> None:
        handler = self._message_handler(topic)
        if handler is not None:
            handler(topic, payload, event_id)

        _log_consumed(topic, payload)

    def _message_handler(self, topic: str):
        if topic in SPOT_FEATURE_TOPICS:
            return self._handle_spot_feature_event
        if topic in FRIEND_REMOVAL_TOPICS:
            return self._handle_friend_removed_event
        return {
            "spot.liked": self._handle_spot_liked_event,
            "review.created": self._handle_review_created_event,
            "user.registered": self._handle_user_registered_event,
            "friend.accepted": self._handle_friend_accepted_event,
            "interaction.recorded": self._handle_interaction_recorded_event,
        }.get(topic)

    def _handle_spot_feature_event(self, topic: str, payload: dict, _event_id: str | None) -> None:
    ...
    def _handle_spot_liked_event(self, _topic: str, payload: dict, event_id: str | None) -> None:
    ...
    def _handle_review_created_event(self, _topic: str, payload: dict, event_id: str | None) -> None:
        spot_id = _payload_spot_id(payload)
        if not spot_id:
    ...
    def _handle_user_registered_event(self, _topic: str, payload: dict, _event_id: str | None) -> None:
    ...
    def _handle_friend_accepted_event(self, _topic: str, payload: dict, _event_id: str | None) -> None:
    ...
    def _handle_friend_removed_event(self, topic: str, payload: dict, _event_id: str | None) -> None:
        requester = payload.get("requesterId") or payload.get("RequesterId")
        addressee = payload.get("addresseeId") or payload.get("AddresseeId")
        if requester and addressee:
    ...
    def _handle_interaction_recorded_event(self, _topic: str, payload: dict, event_id: str | None) -> None:
        occurred_at = _parse_iso_datetime(payload.get("occurredAt"))
        user_id = payload.get("userId")
...
def _log_consumed(topic: str, payload: dict) -> None:
    logger.info(
