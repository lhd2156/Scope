...
test.describe('Phase 20 QA coverage', () => {
  ...
  test('surfaces trip planner edge-case validation and keeps keyboard-only controls operable', async ({ page, atlasApi }) => {
    ...
    for (const interestName of ['Adventure', 'Nature', 'Scenic']) {
      await planner.locator('button.interest-chip').filter({ hasText: interestName }).first().click();
    }

    await planner.locator('[data-test="trip-planner-submit"]').click();

    await expect(planner.getByText('Choose a destination so Atlas can build a route.')).toBeVisible();
    ...
    const budgetSlider = planner.locator('input[type="range"]').first();
  ...
  test('blocks invalid spot coordinates and safely renders HTML-like spot content as text', async ({ page, atlasApi }) => {
    await atlasApi.seedSession(page, {
      id: 'user-1',
      username: 'louisdo',
      email: 'louis@example.com',
    ...
    await form.getByLabel('Description').fill('Premium skyline notes <script>alert("atlas")</script> after dark.');
    ...
    await expect(page.locator('.headline-description')).toContainText('Premium skyline notes <script>alert("atlas")</script> after dark.');
