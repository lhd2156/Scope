...
ATLAS_MEDIA_ROOT = Path(__file__).resolve().parent
...
class AtlasMediaUnavailable(AtlasMediaError):
    pass


class AtlasMediaOperationError(AtlasMediaError):
...
class AtlasMediaBuffer(ctypes.Structure):
    _fields_ = [
...
class AtlasMediaThumbnailOptions(ctypes.Structure):
    _fields_ = [
...
def _configured_library(path: str) -> ctypes.CDLL:
    ...
    library.atlas_media_status_name.argtypes = [ctypes.c_int]
    library.atlas_media_status_name.restype = ctypes.c_char_p
    library.atlas_media_detect_format.argtypes = [
        ctypes.POINTER(ctypes.c_uint8),
        ctypes.c_size_t,
        ctypes.POINTER(ctypes.c_int),
    ]
    library.atlas_media_detect_format.restype = ctypes.c_int
    library.atlas_media_strip_exif.argtypes = [
        ctypes.POINTER(ctypes.c_uint8),
        ctypes.c_size_t,
        ctypes.POINTER(AtlasMediaBuffer),
    ]
    library.atlas_media_strip_exif.restype = ctypes.c_int
    library.atlas_media_generate_thumbnail.argtypes = [
        ctypes.POINTER(AtlasMediaImage),
        ctypes.POINTER(AtlasMediaThumbnailOptions),
        ctypes.POINTER(AtlasMediaImage),
    ]
    library.atlas_media_generate_thumbnail.restype = ctypes.c_int
    ...
    library.atlas_media_free_buffer.argtypes = [ctypes.POINTER(AtlasMediaBuffer)]
    library.atlas_media_free_buffer.restype = None
    library.atlas_media_free_image.argtypes = [ctypes.POINTER(AtlasMediaImage)]
    library.atlas_media_free_image.restype = None
    return library
...
def load_library(*, allow_build: bool = True) -> ctypes.CDLL:
    ...
    if not output.exists():
        if not allow_build:
            raise AtlasMediaUnavailable(f'Compiled atlas_media library not found at {output}')
        if not compiler_available():
            raise AtlasMediaUnavailable('No C compiler found in PATH (expected cl or gcc).')
        try:
            ...
            raise AtlasMediaUnavailable('Failed to build atlas_media native library.') from exc
        if not output.exists():
            raise AtlasMediaUnavailable(f'Compiled atlas_media library not found at {output}')
...
def status_name(status: int) -> str:
    ...
    if output.exists():
        try:
            value = _configured_library(str(output)).atlas_media_status_name(status)
            if value:
...
def _raise_for_status(operation: str, status: int) -> None:
    if status != STATUS_OK:
        raise AtlasMediaOperationError(operation, status)
...
def detect_format(payload: bytes) -> str | None:
    ...
    status = load_library().atlas_media_detect_format(_byte_buffer(data), len(data), ctypes.byref(detected_format))
...
def strip_exif(payload: bytes) -> bytes:
    data = _require_non_empty_payload(payload)
    output = AtlasMediaBuffer()
    library = load_library()
    status = library.atlas_media_strip_exif(_byte_buffer(data), len(data), ctypes.byref(output))
    try:
        _raise_for_status('strip_exif', status)
        return ctypes.string_at(output.data, output.length) if bool(output.data) else b''
    finally:
        if bool(output.data):
            library.atlas_media_free_buffer(ctypes.byref(output))
...
def generate_thumbnail_pixels(
    ...
    *,
    ...
    options = AtlasMediaThumbnailOptions(
        max_width=max_width,
        max_height=max_height,
        channels=output_channels,
    )
    output_image = AtlasMediaImage()
