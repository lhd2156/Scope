...
def compiled_library() -> ctypes.CDLL:
    if shutil.which('cl') is None and shutil.which('gcc') is None:
        pytest.skip('No C compiler found in PATH, skipping native atlas_media detection tests.')
...
def _call_detect(library: ctypes.CDLL, payload: bytes) -> tuple[int, int]:
    ...
    status = library.atlas_media_detect_format(buffer, len(payload), ctypes.byref(detected_format))
...
def test_detect_format_rejects_invalid_arguments(compiled_library: ctypes.CDLL) -> None:
    ...
    assert compiled_library.atlas_media_detect_format(None, 0, ctypes.byref(detected_format)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.atlas_media_detect_format(None, 10, ctypes.byref(detected_format)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.atlas_media_detect_format((ctypes.c_uint8 * 3)(0xFF, 0xD8, 0xFF), 3, None) == STATUS_INVALID_ARGUMENT
