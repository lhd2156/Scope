...
```powershell
powershell -File C:\Users\dongu\scope\scripts\load-lessons.ps1 -Agent core
```
Also read `scope_architecture.tex` for specifications. You CANNOT proceed without doing this step.

## Step 1: Read Progress
Read `core/PROGRESS.md` in C:\Users\dongu\scope. Find your current status and the first unchecked task (`- [ ]`).
...
Read `core/agents.md` for the detailed task instructions for that task number. Then DO the work. Actually create files, run commands, build things inside `Scope.Core/`.

## Step 4: Update Progress
- Mark the completed task as `- [x]` in `core/PROGRESS.md`
...
**If task succeeded:**
```powershell
powershell -File C:\Users\dongu\scope\scripts\complete-task.ps1 -Message "feat(core): your description" -Result "success" -Lesson "what you learned, if anything" -Agent "core"
```

**If task failed:**
```powershell
powershell -File C:\Users\dongu\scope\scripts\complete-task.ps1 -Message "fix(core): attempted description" -Result "failure" -Lesson "what went wrong and why" -Agent "core"
```
...
## Step 6: Self-Check
- Am I in scope? Only modify files inside `Scope.Core/`. If editing anything else, STOP.
- Am I following `scope_architecture.tex`?
- Did I run tests? (`dotnet test`)
...
- If ALL tasks are done → set `## Status` to `COMPLETE`, push branch: `git push origin feature/core-platform`, then stop.
