...
export interface AtlasWasmModuleInfo {
  version: string;
...
export interface AtlasWasmCoordinateInput {
  id?: string;
  latitude?: number;
  lat?: number;
  longitude?: number;
...
export interface AtlasWasmViewport {
  west: number;
  south: number;
  east: number;
  north: number;
...
export interface AtlasWasmClusterOptions {
  radiusPx?: number;
  minPoints?: number;
  includeSingles?: boolean;
}

export interface AtlasWasmDistanceResult {
  valid: boolean;
  fromId: string;
  toId: string;
  kilometers: number;
...
export interface AtlasWasmClusterResult {
  id: string;
  clustered: boolean;
  pointCount: number;
  latitude: number;
...
export interface AtlasWasmHullPoint {
  id: string;
  latitude: number;
  longitude: number;
  screenX: number;
...
export interface AtlasWasmHullResult {
  valid: boolean;
  pointCount: number;
...
interface RawAtlasWasmModuleInfo {
  version: string;
  algorithmsReady: boolean;
  status: string;
}

interface AtlasWasmBindings {
  ping: () => string;
  getModuleInfo: () => RawAtlasWasmModuleInfo;
  calculateHaversineDistance: (
    from: AtlasWasmCoordinateInput,
    to: AtlasWasmCoordinateInput,
  ) => AtlasWasmDistanceResult;
  clusterViewportPoints: (
    points: AtlasWasmCoordinateInput[],
    viewport: AtlasWasmViewport,
    options?: AtlasWasmClusterOptions,
  ) => AtlasWasmClusterResult[] | ArrayLike<AtlasWasmClusterResult>;
...
interface AtlasWasmFactoryConfig {
  locateFile?: (path: string) => string;
}

type AtlasWasmFactory = (config?: AtlasWasmFactoryConfig) => Promise<AtlasWasmBindings> | AtlasWasmBindings;

interface AtlasWasmRuntime {
  mode: RuntimeMode;
  bindings: AtlasWasmBindings;
  fallbackReason?: string;
}
...
let runtimePromise: Promise<AtlasWasmRuntime> | null = null;
...
function parseCoordinate(
  input: AtlasWasmCoordinateInput | null | undefined,
  fallbackId: string,
...
function parseViewport(viewport: AtlasWasmViewport): AtlasWasmViewport {
...
function parseClusterOptions(options: AtlasWasmClusterOptions = {}): Required<AtlasWasmClusterOptions> {
  return {
...
function buildProjectionContext(viewport: AtlasWasmViewport): ViewportProjectionContext {
...
function createSingletonResult(point: ProjectedPoint): AtlasWasmClusterResult {
  return {
    id: point.id,
    clustered: false,
    pointCount: 1,
...
function clusterViewportPointsFallback(
  ...
  const results: AtlasWasmClusterResult[] = [];
...
function buildViewportConvexHullFallback(
  ...
  const emptyResult: AtlasWasmHullResult = {
    valid: false,
    pointCount: 0,
    hullPointCount: 0,
    latitude: 0,
...
function createFallbackBindings(): AtlasWasmBindings {
  return {
    ping: () => 'atlas-wasm-js-fallback-ready',
    getModuleInfo: () => ({
...
function sanitizeDistanceResult(result: AtlasWasmDistanceResult): AtlasWasmDistanceResult {
  return {
...
function sanitizeClusterResult(result: AtlasWasmClusterResult): AtlasWasmClusterResult {
  return {
    ...result,
...
function sanitizeHullResult(result: AtlasWasmHullResult): AtlasWasmHullResult {
  return {
    ...result,
    pointIds: normalizeArray(result.pointIds),
...
async function loadWasmBindings(): Promise<AtlasWasmBindings> {
  const modulePath = `${resolveWasmAssetBasePath()}atlas_wasm.js`;
  const importedModule = await import(/* @vite-ignore */ modulePath);
  const factory = (
    (typeof importedModule.default === 'function' && importedModule.default)
    || (typeof importedModule.createAtlasWasmModule === 'function' && importedModule.createAtlasWasmModule)
  ) as AtlasWasmFactory | undefined;

  if (!factory) {
    throw new Error('Atlas WASM module did not expose a supported factory.');
  }
...
async function getRuntime(): Promise<AtlasWasmRuntime> {
  if (!runtimePromise) {
    runtimePromise = loadWasmBindings()
      ...
      .catch((error: unknown) => ({
        ...
        fallbackReason: error instanceof Error ? error.message : 'Unknown Atlas WASM loader failure.',
      }));
  }
...
export async function preloadAtlasWasmRuntime(): Promise<void> {
  await getRuntime();
}

export async function pingAtlasWasmRuntime(): Promise<string> {
  const runtime = await getRuntime();
  return runtime.bindings.ping();
}

export async function getAtlasWasmModuleInfo(): Promise<AtlasWasmModuleInfo> {
  const runtime = await getRuntime();
...
export async function calculateHaversineDistance(
  from: AtlasWasmCoordinateInput,
  to: AtlasWasmCoordinateInput,
): Promise<AtlasWasmDistanceResult> {
  const runtime = await getRuntime();
...
export async function clusterViewportPoints(
  ...
  viewport: AtlasWasmViewport,
  options?: AtlasWasmClusterOptions,
): Promise<AtlasWasmClusterResult[]> {
  const runtime = await getRuntime();
