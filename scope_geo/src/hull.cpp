...
bool coordinate_less(const scope::geo::Coordinate& left, const scope::geo::Coordinate& right) {
    return std::tie(left.longitude, left.latitude) < std::tie(right.longitude, right.latitude);
}

double cross_product(
    const scope::geo::Coordinate& origin,
    const scope::geo::Coordinate& point_a,
    const scope::geo::Coordinate& point_b) {
...
void append_hull_point(std::vector<scope::geo::Coordinate>& chain, const scope::geo::Coordinate& point) {
    while (chain.size() >= 2
