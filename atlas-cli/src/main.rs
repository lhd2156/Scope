        ...
        Self {
            services: vec![
                ...
                ServiceTarget {
                    ...
                    health_url: env_or_default(
                        "ATLAS_INTEL_HEALTH_URL",
                        "http://localhost:5003/api/intel/health",
                    ),
                },
                ServiceTarget {
                    name: "atlas-metrics".to_string(),
                    health_url: env_or_default(
                        "ATLAS_METRICS_HEALTH_URL",
                        "http://localhost:9090/healthz",
                    ),
                },
            ],
            seed_directory: env_path_or_default("ATLAS_SEED_DIRECTORY", "scripts/sql"),
            env_file: env_path_or_default("ATLAS_ENV_FILE", ".env"),
            env_example_file: env_path_or_default("ATLAS_ENV_EXAMPLE_FILE", ".env.example"),
        }
    }
}
...
async fn run_health(args: HealthArgs, context: &AppContext) -> CliResult {
    ...
    print_banner("Atlas service health");
...
async fn run_seed(args: SeedArgs, context: &AppContext) -> CliResult {
    ...
    print_banner("Atlas SQL seeding");
...
fn resolve_database_config(
    ...
    let host = resolve_value(&["ATLAS_DB_HOST", "DB_HOST"], &env_map)
        .or_else(|| Some(connection_host))
        .unwrap_or_else(|| "localhost".to_string());
    let port = resolve_value(&["ATLAS_DB_PORT", "DB_PORT"], &env_map)
        .map(|value| parse_u16(&value, "database port"))
        .transpose()?
        .or(connection_port)
        .unwrap_or(1433);
    let database = resolve_value(&["ATLAS_DB_NAME", "DB_NAME"], &env_map)
        .or_else(|| {
            connection_map
                .get("database")
                .cloned()
    ...
    let user = resolve_value(&["ATLAS_DB_USER", "DB_USER"], &env_map)
        .or_else(|| {
            connection_map
                .get("user id")
                .cloned()
    ...
    let password = resolve_value(&["ATLAS_DB_PASSWORD", "DB_PASSWORD", "SA_PASSWORD"], &env_map)
        .or_else(|| {
            connection_map
                .get("password")
                .cloned()
    ...
    let trust_cert = resolve_value(&["ATLAS_DB_TRUST_CERT"], &env_map)
...
async fn run_deploy_validate(args: DeployValidateArgs, _context: &AppContext) -> CliResult {
    print_banner("Atlas deploy validation");
...
async fn run_env_check(args: EnvCheckArgs, _context: &AppContext) -> CliResult {
    print_banner("Atlas env contract");
...
fn looks_like_placeholder(value: &str) -> bool {
    ...
    normalized.is_empty()
        ...
        || normalized.contains("atlas_dev_2026")
}
...
mod tests {
    ...
    fn parses_nested_subcommands() {
        let cli =
            Cli::try_parse_from(["atlas", "deploy", "validate"]).expect("deploy validate should parse");
...
",
    ...
    fn parses_sql_server_connection_strings() {
        let parsed = parse_connection_string(
            "Server=sqlserver,1433;Database=AtlasDb;User Id=sa;Password=secret;TrustServerCertificate=True;",
        );

        assert_eq!(parsed.get("server"), Some(&"sqlserver,1433".to_string()));
        assert_eq!(parsed.get("database"), Some(&"AtlasDb".to_string()));
