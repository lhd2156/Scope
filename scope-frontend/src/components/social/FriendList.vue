      ...
      >
        ...
        <div class="friend-card__body">
          <div>
            ...
            <p class="friend-card__username">@{{ friend.username || 'scopetraveler' }}</p>
          </div>

          <p class="friend-card__location">
            <ScopeIcon name="pin" />
            <span>{{ friend.homeBase || 'Scope traveler' }}</span>
          </p>
          ...
          <p class="friend-card__route">{{ friend.nextAdventure || 'Ready for the next Scope route.' }}</p>
        </div>
...
const resolvedTitle = computed(() => props.title ?? `${props.friends.length} friend${props.friends.length === 1 ? '' : 's'} on Scope`);
