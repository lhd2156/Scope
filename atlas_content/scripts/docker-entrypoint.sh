...
# Fall back to Django's dev server only when explicitly requested (ATLAS_CONTENT_DEV_SERVER=true).
if [ "${ATLAS_CONTENT_DEV_SERVER:-false}" = "true" ]; then
...
exec gunicorn atlas_content.wsgi:application \
    --bind 0.0.0.0:8000 \
