    ...
    <div class="member-grid">
      <article v-for="member in members" :key="member.id" class="surface-card member-card">
        ...
        <div class="member-copy">
          ...
          <span>{{ roleLabel(member.status, member.inviteStatus) }}</span>
        </div>
      </article>
    </div>
  </section>
...
function roleLabel(status?: string, inviteStatus?: string): string {
  if (!status) {
    return 'Trip member';
  }

  const role = status.charAt(0).toUpperCase() + status.slice(1);
  return inviteStatus === 'pending' ? `${role} invite pending` : role;
}
</script>
