...
export const useTripsStore = defineStore('trips', () => {
  ...
  async function fetchMembers(tripId = selectedTrip.value?.id ?? '') {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not load trip members right now.');
      throw nextError;
    } finally {
      loading.value = false;
    }
  ...
  async function updateTrip(tripId: string, input: TripMutationInput): Promise<Trip> {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not update that trip right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
  ...
  async function deleteTrip(tripId: string): Promise<void> {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not delete that trip right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
  ...
  async function addSpot(tripId: string, tripSpot: TripSpot): Promise<Trip> {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not add that stop right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
  ...
  async function removeSpot(tripId: string, spotId: string): Promise<Trip> {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not remove that stop right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
  ...
  async function reorderSpots(tripId: string, input: ReorderTripSpotsInput): Promise<Trip> {
    ...
    try {
      ...
      error.value = toAsyncErrorMessage(nextError, 'Atlas could not reorder those stops right now.');
      throw nextError;
    } finally {
      saving.value = false;
    }
