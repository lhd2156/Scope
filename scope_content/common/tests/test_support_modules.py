...
from common.kafka_consumer import ScopeKafkaConsumer
...
def test_asgi_and_wsgi_modules_expose_applications():
    asgi_module = importlib.reload(importlib.import_module('scope_content.asgi'))
    wsgi_module = importlib.reload(importlib.import_module('scope_content.wsgi'))
...
def test_kafka_consumer_initializes_and_polls_records(monkeypatch):
    ...
    assert captured['kwargs']['group_id'] == 'scope-content'
