...
describe('demo photo surfaces', () => {
  beforeEach(() => {
    // FeedItem's engagement actions read from the auth + toast pinia
    // stores, so keep a fresh instance available for every test in the
    // block even though only the FeedItem cases strictly need it.
  ...
  it('renders the neutral silhouette placeholder for ProfileHeader when an avatar is missing', () => {
