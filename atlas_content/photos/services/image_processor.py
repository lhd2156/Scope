...
ATLAS_MEDIA_BINDINGS_PATH = Path(__file__).resolve().parents[3] / 'atlas_media' / 'python_bindings.py'
...
def _native_bindings() -> ModuleType | None:
    if not ATLAS_MEDIA_BINDINGS_PATH.exists():
        return None

    spec = importlib.util.spec_from_file_location('atlas_media_python_bindings', ATLAS_MEDIA_BINDINGS_PATH)
