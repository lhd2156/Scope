...
def test_route_map_marks_all_protected_endpoints_with_require_auth(app):
    protected_rules = [rule for rule in app.url_map.iter_rules() if rule.rule.startswith("/api/intel") and rule.rule != "/api/intel/health"]
    assert protected_rules

    for rule in protected_rules:
        view_function = app.view_functions[rule.endpoint]
        assert getattr(view_function, "_atlas_require_auth", False), f"{rule.rule} is missing @require_auth"

    assert not getattr(app.view_functions["health.health"], "_atlas_require_auth", False)
