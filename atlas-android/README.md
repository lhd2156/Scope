# Atlas Android (`atlas-android/`)

Native Android client for the Atlas adventure platform — Kotlin + Jetpack
...
|---|---|---|
| Atlas Core (C# / ASP.NET Core) | `ATLAS_CORE_BASE_URL` | `CoreApi` |
| Atlas Content (Django) | `ATLAS_CONTENT_BASE_URL` | `ContentApi` |
| Atlas Intel (Flask) | `ATLAS_INTEL_BASE_URL` | `IntelApi` |
...
│       ├── main/
│       │   ├── AndroidManifest.xml
│       │   ├── kotlin/com/atlas/mobile/
│       │   │   ├── AtlasApplication.kt        # @HiltAndroidApp
...
- **Auto-refresh on 401.** `AtlasAuthenticator` (OkHttp `Authenticator`
  contract) catches 401s, refreshes the token once, and retries the failed
  request transparently — never re-entrant.
- **Design tokens as Kotlin.** `AtlasTokens` mirrors
  `atlas-assets/design-tokens.css`, so palette drift between web / iOS /
  Android is impossible: every platform reads from the same table.
- **Dark by default.** Atlas ships dark-first; `AtlasTheme` provides a light
...
1. Open `atlas-android/` directly (Android Studio Giraffe+ with AGP 8.5).
...
1. `-P<key>=<value>` Gradle property: `.\gradlew.bat assembleDebug -PATLAS_CORE_BASE_URL=https://core.staging.atlas.app/`
2. `local.properties` (gitignored): `ATLAS_CORE_BASE_URL=https://core.staging.atlas.app/`
3. Environment variables: `set ATLAS_CORE_BASE_URL=https://...`
...
|---|---|---|
| `ATLAS_CORE_BASE_URL` | Core (auth / friends / notifications / live) | `http://10.0.2.2:5080/` |
| `ATLAS_CONTENT_BASE_URL` | Content (spots / trips / feed / photos) | `http://10.0.2.2:8000/` |
| `ATLAS_INTEL_BASE_URL` | Intel (itineraries / vibe / recommendations) | `http://10.0.2.2:5000/` |
| `ATLAS_MAP_STYLE_URL` | Vector tile style for the MapLibre map | MapLibre demo tiles |
...
- `toAtlasError()` — HTTP 401/404/500/IOException/unknown mapping
