...
/// Client for the C# ASP.NET Core backend (`Atlas.Core`). Handles auth,
...
public actor CoreService {
    ...
    public func login(email: String, password: String) async throws -> AuthPayload {
        ...
        await tokens.save(AtlasTokens(accessToken: payload.accessToken, refreshToken: payload.refreshToken))
        return payload
    }
    ...
    public func me() async throws -> AtlasUser {
        try await client.get("/api/core/auth/me")
    }

    // MARK: - Users

    public func user(id: String) async throws -> AtlasUser {
        try await client.get("/api/core/users/\(id)")
    }

    public func searchUsers(query: String) async throws -> [AtlasUser] {
    ...
    public func notifications(page: Int = 1, pageSize: Int = 20) async throws -> [AtlasNotification] {
