...
def compiled_library() -> ctypes.CDLL:
    if shutil.which('cl') is None and shutil.which('gcc') is None:
        pytest.skip('No C compiler found in PATH, skipping native atlas_media thumbnail tests.')
...
def test_generate_thumbnail_rejects_invalid_arguments(compiled_library: ctypes.CDLL) -> None:
    output = AtlasMediaImage()
    options = AtlasMediaThumbnailOptions(max_width=2, max_height=2, channels=0)
    ...
    assert compiled_library.atlas_media_generate_thumbnail(None, ctypes.byref(options), ctypes.byref(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.atlas_media_generate_thumbnail(ctypes.byref(image), None, ctypes.byref(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.atlas_media_generate_thumbnail(ctypes.byref(image), ctypes.byref(options), None) == STATUS_INVALID_ARGUMENT

    zero_bounds = AtlasMediaThumbnailOptions(max_width=0, max_height=2, channels=0)
    assert compiled_library.atlas_media_generate_thumbnail(ctypes.byref(image), ctypes.byref(zero_bounds), ctypes.byref(output)) == STATUS_INVALID_ARGUMENT
