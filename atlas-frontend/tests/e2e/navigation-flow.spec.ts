...
const publicRoutes: RouteExpectation[] = [
  {
    ...
    path: '/',
    assert: async (page) => {
      await expect(page.getByRole('heading', { level: 1, name: 'Your Adventures, Mapped.' })).toBeVisible();
...
const protectedRoutes: RouteExpectation[] = [
  {
    ...
    path: '/friends',
    assert: async (page) => {
      await expect(page.getByRole('heading', { level: 1, name: 'Build your Atlas travel circle.' })).toBeVisible();
...
test.describe('Atlas route navigation and guard coverage', () => {
  test('renders every public route and the 404 fallback without an authenticated session', async ({ page }) => {
    ...
    await test.step('renders the 404 fallback for unknown paths', async () => {
      ...
      await expect(page.getByRole('heading', { name: 'This trail does not exist in Atlas' })).toBeVisible();
      await expect(page.getByText('Page not found')).toBeVisible();
      await expect(page.getByRole('link', { name: 'Return home' })).toHaveAttribute('href', '/');
    });
  });

  test('redirects every protected route to login and preserves the redirect target for guests', async ({ page, atlasApi }) => {
    await atlasApi.clearSession(page);
