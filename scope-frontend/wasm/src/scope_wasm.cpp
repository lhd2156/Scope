...
const std::unordered_set<std::string> kScopeAiLexerStopWords = {
  "a", "an", "and", "around", "at", "can", "could", "for", "from", "in", "into", "it",
  "me", "my", "of", "on", "please", "pls", "that", "the", "this", "to", "with", "you",
};

const std::unordered_map<std::string, std::string> kScopeAiLexerKeywordTypes = {
  {"map", "map_keyword"},
  {"zoom", "zoom_keyword"},
...
const std::unordered_map<std::string, std::string> kScopeAiLexerAliases = {
  {"mapp", "map"},
  {"mpa", "map"},
  {"maap", "map"},
  {"mappp", "map"},
...
std::string normalizeScopeAiLexeme(const std::string& value) {
  ...
  const auto alias = kScopeAiLexerAliases.find(normalized);
  if (alias != kScopeAiLexerAliases.end()) {
    return alias->second;
  }
...
bool isScopeAiEmailLexeme(const std::string& value) {
...
bool isScopeAiHandleLexeme(const std::string& value) {
...
bool isScopeAiNumberLexeme(const std::string& normalized) {
  bool sawDigit = false;
...
std::string classifyScopeAiLexeme(const std::string& value, const std::string& normalized) {
  if (isScopeAiNumberLexeme(normalized)) {
    return "number";
  }
  ...
  const auto keywordType = kScopeAiLexerKeywordTypes.find(normalized);
  if (keywordType != kScopeAiLexerKeywordTypes.end()) {
    return keywordType->second;
  }

  if (kScopeAiLexerStopWords.find(normalized) != kScopeAiLexerStopWords.end()) {
    return "filler";
  }
...
bool isScopeAiPlaceCandidateToken(const ScopeAiLexToken& token) {
  return (token.type == "word" || token.type == "number") && token.normalized.size() > 1;
}

emscripten::val createScopeAiLexTokenObject(const ScopeAiLexToken& token) {
...
void appendScopeAiPlaceSpanTokens(emscripten::val& results, const std::vector<ScopeAiLexToken>& tokens, int& outputIndex) {
  std::size_t index = 0;
  ...
  while (index < tokens.size()) {
    ...
    while (index < tokens.size() && isScopeAiPlaceCandidateToken(tokens[index])) {
      endIndex = index;
      ++index;
    }
    ...
    results.set(outputIndex++, createScopeAiLexTokenObject(span));
  }
}
...
emscripten::val lexScopeAiCommandText(const std::string& input) {
  std::vector<ScopeAiLexToken> tokens;
  auto results = emscripten::val::array();
  std::size_t index = 0;
  int outputIndex = 0;

  while (index < input.size()) {
    while (index < input.size() && !isScopeAiLexemeCharacter(input[index])) {
      ++index;
    }
    ...
    const auto normalized = normalizeScopeAiLexeme(value);
    if (normalized.empty()) {
      continue;
    }

    ScopeAiLexToken token;
    token.type = classifyScopeAiLexeme(value, normalized);
    token.value = value;
    token.normalized = normalized;
    token.start = static_cast<int>(start);
    token.end = static_cast<int>(end);
    tokens.push_back(token);
    results.set(outputIndex++, createScopeAiLexTokenObject(token));
  }

  appendScopeAiPlaceSpanTokens(results, tokens, outputIndex);
  return results;
}
