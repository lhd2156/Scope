...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{Scope --- Comprehensive Architecture Document}}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\title{\Huge\textbf{Scope}\\[10pt]\Large Comprehensive Architecture \& Implementation Guide\\[5pt]\normalsize Real-World Adventure Platform --- Polyglot Microservices}
\author{Louis Do}
\date{March 2026}
...
\subsection{What is Scope?}
Scope is a real-world adventure platform where users document, discover, and plan experiences on an interactive map. Users drop pins at locations they visit, upload photos, write stories, tag the vibe, and share adventures with friends. Over time, each user builds a personal map of every place they have explored. Other users discover these spots, and an AI engine plans optimized itineraries from community data.
...
Scope follows a polyglot microservices architecture with three backend services, each written in a different framework chosen for its strengths:
...
This is the primary service and the heart of Scope. It handles authentication, user management, real-time features, friend system, and notifications.
...
jobs:
  test-core:
    ...
    steps:
      ...
      - run: dotnet test Scope.Core.Tests
  ...
  test-content:
    ...
    steps:
      ...
      - run: pip install -r scope_content/requirements.txt
      - run: pytest scope_content/
  ...
  test-intel:
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with: { python-version: '3.12' }
      - run: pip install -r scope_intel/requirements.txt
      - run: pytest scope_intel/
  ...
  test-frontend:
    ...
    steps:
      ...
      - run: cd scope-frontend && npm ci && npm run test
  ...
  deploy:
    ...
    steps:
      ...
      - run: |
          docker build -t scope-core ./Scope.Core
          docker build -t scope-content ./scope_content
          docker build -t scope-intel ./scope_intel
          docker build -t scope-frontend ./scope-frontend
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Create a new repository called \texttt{Scope} (public or private)
...
\begin{lstlisting}[style=code,language=bash]
# Clone the repo
git clone https://github.com/lhd2156/Scope.git
cd Scope
...
dotnet ef database update --project Scope.Core
python scope_content/manage.py migrate
python scope_intel/manage.py migrate

# Seed sample data
python scripts/seed_data.py

# Frontend dev server
cd scope-frontend && npm install && npm run dev
...
\begin{lstlisting}[style=code]
Scope/
  Scope.Core/                 # C# / ASP.NET Core service
    Scope.Core.API/
    Scope.Core.Domain/
    Scope.Core.Infrastructure/
    Scope.Core.Tests/
    Scope.Core.sln
    Dockerfile
  scope_content/              # Python / Django service
    scope_content/
    spots/
    trips/
    photos/
    ...
    requirements.txt
    Dockerfile
  scope_intel/                # Python / Flask service
    app.py
    config.py
    routes/
    services/
    ...
    requirements.txt
    Dockerfile
  scope-frontend/             # Vue.js + TypeScript
    src/
    public/
    tests/
    package.json
...
DB_NAME=scope
DB_USER=sa
DB_PASSWORD=Scope_Dev_2026!

# ============ CORE PLATFORM (C#) ============
CORE_ConnectionStrings__Default=
    Server=sqlserver,1433;Database=scope;
    User Id=sa;Password=Scope_Dev_2026!;
...
DJANGO_DATABASE_URL=mssql://sa:Scope_Dev_2026!
    @sqlserver:1433/scope

# ============ INTELLIGENCE API (Flask) ============
FLASK_SECRET_KEY=flask-insecure-change-me-in-prod
FLASK_DEBUG=True
FLASK_DATABASE_URL=mssql://sa:Scope_Dev_2026!
    @sqlserver:1433/scope
...
WORKDIR /src
COPY Scope.Core.sln .
COPY Scope.Core.API/*.csproj Scope.Core.API/
COPY Scope.Core.Domain/*.csproj Scope.Core.Domain/
COPY Scope.Core.Infrastructure/*.csproj
    Scope.Core.Infrastructure/
RUN dotnet restore
COPY . .
RUN dotnet publish Scope.Core.API -c Release
    -o /app/publish
...
HEALTHCHECK CMD curl -f http://localhost/api/core/health
    || exit 1
ENTRYPOINT ["dotnet", "Scope.Core.API.dll"]
\end{lstlisting}
...
CMD ["gunicorn", "scope_content.wsgi:application",
...
\begin{itemize}
  ...
  \item Refer to the \texttt{design-tokens.css} file in \texttt{scope-assets/} for the complete token list.
\end{itemize}
...
Agent 1: Core & Auth, real-time, users & \texttt{Scope.Core/} & C\# \\
Agent 2: Content & Spots, trips, social & \texttt{scope\_content/} & Python \\
Agent 3: Intel & AI, recommendations & \texttt{scope\_intel/} & Python \\
Agent 4: Frontend & UI, maps, state & \texttt{scope-frontend/} & TypeScript \\
\hline
\end{tabular}
...
\begin{enumerate}
  \item \textbf{File boundaries are sacred.} Agent 1 NEVER touches \texttt{scope\_content/}. Agent 2 NEVER touches \texttt{Scope.Core/}. Violations will cause merge conflicts.
...
The following are four resume-ready bullet points for Scope, written in XYZ format matching the style of existing resume projects:
...
\subsection{scope\_content/requirements.txt}
\begin{lstlisting}[style=code]
Django==5.1
...
\subsection{scope\_intel/requirements.txt}
\begin{lstlisting}[style=code]
Flask==3.1.0
flask-cors==5.0.0
...
\subsection{scope-frontend/package.json (Key Dependencies)}
\begin{lstlisting}[style=code,language=Java]
{
  "name": "scope-frontend",
  "version": "1.0.0",
...
\begin{lstlisting}[style=code,language=bash]
#!/bin/bash
# Create all Kafka topics for Scope
...
until docker-compose exec -T sqlserver \
  ...
  -S localhost -U sa -P "Scope_Dev_2026!" \
