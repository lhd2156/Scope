...
def compiled_library() -> ctypes.CDLL:
    if shutil.which('cl') is None and shutil.which('gcc') is None:
        pytest.skip('No C compiler found in PATH, skipping native atlas_media EXIF tests.')
...
def _call_strip(library: ctypes.CDLL, payload: bytes) -> tuple[int, bytes]:
    ...
    status = library.atlas_media_strip_exif(buffer, len(payload), ctypes.byref(output))
...
def test_strip_exif_rejects_invalid_arguments(compiled_library: ctypes.CDLL) -> None:
    ...
    assert compiled_library.atlas_media_strip_exif(None, 0, ctypes.byref(output)) == STATUS_INVALID_ARGUMENT
    assert compiled_library.atlas_media_strip_exif(jpeg_header, 3, None) == STATUS_INVALID_ARGUMENT
